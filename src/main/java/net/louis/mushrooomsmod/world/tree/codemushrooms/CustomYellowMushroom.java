...
public abstract class CustomYellowMushroom extends Feature<HugeMushroomFeatureConfig> {
    ...
    public boolean generate(FeatureContext<HugeMushroomFeatureConfig> context) {
        ...
        int height = Random.create().nextBetween(7,17);

        if (!this.canGenerate(world, blockPos, 10, 10, mutable,hugeMushroomFeatureConfig)) {
            return false;
        }
        int large = Random.create().nextBetween(4,8);
        Integer[] coordinates = generateTrunk(world,random,blockPos,mutable,height,hugeMushroomFeatureConfig);
        this.generateCap(world, random, blockPos,height,mutable , hugeMushroomFeatureConfig, large, coordinates );
        return true;
    }
    ...
    protected abstract void generateCap(WorldAccess var1, Random var2, BlockPos var3, int var4, BlockPos.Mutable var5, HugeMushroomFeatureConfig var6, int large, Integer[] coordinates);
