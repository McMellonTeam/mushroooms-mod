...
public class CustomYellowMushroomFeature extends CustomYellowMushroom{
    ...
    protected Integer[] generateTrunk(WorldAccess world, Random random, BlockPos pos, BlockPos.Mutable mutable, int height, HugeMushroomFeatureConfig config) {
        ...
        System.out.println(randomx +"  " + randomz+"   "+lenght+"    "+theta+"   "+phi);

        return coordinates;
    }

    @Override
    protected void generateCap(WorldAccess world, Random random, BlockPos pos, int height, BlockPos.Mutable mutable, HugeMushroomFeatureConfig config, int large, Integer[] coordinates) {
        ...
        for (float secondlarge = 0; secondlarge <= large; secondlarge = secondlarge + 0.25f){
            for (double i = 0; i <= Math.PI / 2; i = i + Math.PI / (4 * large)) {
                for (double j = -Math.PI; j <= Math.PI; j = j + Math.PI / (4 * large)) {
                    ...
                    double y = secondlarge * Math.sin(i);
                    mutable.set(pos, (int) x + randomx, (int) (y + height-large-1), (int) z + randomz);
